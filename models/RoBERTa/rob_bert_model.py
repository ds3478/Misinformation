...
class RobertaMisinfoDataset(torch.utils.data.Dataset):
    def __init__(self, tokenizer:RobertaTokenizer, data_X_df:pd.DataFrame, data_y_df:pd.DataFrame, target_text_factor:str, max_sequence_length:int):
...
class RobertaMisinfoPredictionDataset(torch.utils.data.Dataset):
    def __init__(self, tokenizer:RobertaTokenizer, data_X_lst:list, max_sequence_length:int):
        self.tweets =  [
            tokenizer(
                tweet,
                padding = 'max_length',           # pad each sequence to the specified maximum lengt
...
class RobertaCovidMisinfoClassifier(nn.Module):
    def __init__(self, max_seq_length:int, drop_prob:float = 0.0, bert_weights_name:str = 'bert-base-uncased'):
        
        super(RobertaCovidMisinfoClassifier, self).__init__()
        
        self.model = RobertaModel.from_pretrained(bert_weights_name)
    ...
    def predict_multiple(self, texts:list, stop_words:list = None, verbose:bool = False) -> list:
        X_test_bert_ds = RobertaMisinfoPredictionDataset(
...
def get_trained_roberta_classifier(target_text_factor:str, X_train:pd.DataFrame, y_train:pd.DataFrame, X_valid:pd.DataFrame, y_valid:pd.DataFrame, epochs:int, max_sequence_length:int, batch_size:int, developer_params:{} = None, bert_weights_name:str = 'roberta-base') -> RobertaCovidMisinfoClassifier:

    epoch_step_comp_size = (int)(np.round((len(X_train)) / (float)(batch_size)))
    epoch_step_comp_size = np.max([1, epoch_step_comp_size])
    
    bert_tokenizer = RobertaTokenizer.from_pretrained(bert_weights_name)

    X_train_bert_ds = RobertaMisinfoDataset(
        tokenizer = bert_tokenizer,
        data_X_df = X_train,
        data_y_df = y_train,
    ...
    X_valid_bert_ds = RobertaMisinfoDataset(
        tokenizer = bert_tokenizer,
        data_X_df = X_valid,
        data_y_df = y_valid,
        target_text_factor = target_text_factor,
    ...
    bert_classifier = RobertaCovidMisinfoClassifier(
